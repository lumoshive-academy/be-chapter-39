...
func main() {
	...
	// implementation one to many
	OneToManyGrom(db, int(user.ID))
}

func OneToManyGrom(db *gorm.DB, id int) {
...
func ManyToManyGorm(db *gorm.DB) {
	// Migrasi schema
	db.AutoMigrate(&Project{})

	// Membuat user dan project baru
	user1 := User{Name: "Doni Cagur", Email: "doni.cagur@example.com"}
	user2 := User{Name: "Fendi Irawan", Email: "fendi.irawan@example.com"}
